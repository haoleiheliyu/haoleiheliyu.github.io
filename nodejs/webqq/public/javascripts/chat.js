$(document).ready(function() {
    ...
    socket.on('online',function (data) {
        if(data.user!=from)
        {
            var sys='<div style="color: #f00">现在是'+now()+'用户'+from+'上线了</div>';
            console.log("aa");
        }
        else
        {
            var sys = '<div style="color: #f00">现在是' + now() + ':你进入了聊天室!</div>';
            console.log("bb");

        }
        $('#contents').append(sys+'<br>');
        //刷新用户在线列表
        shuaxinliebiao(data.users);
        //显示正在对谁说话
        showsayto();
